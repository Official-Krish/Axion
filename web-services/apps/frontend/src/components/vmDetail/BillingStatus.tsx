...
import { Loader2, Plus, Wallet, Zap } from "lucide-react";
  ...
  const handleTopUp = async () => {
    ...
    try {
      ...
      watch(tx.signature, {
        ...
        onFailed: () => {
          setTxStatus("failed");
          toast.error("Top-up transaction failed on-chain", {
            position: "top-right",
          });
        },
      });
      ...
      setTxStatus("failed");
      toast.error("Failed to top up escrow balance", { position: "top-right" });
    }
  };
  ...
  useEffect(() => {
    const calculateEscrowDuration = async () => {
      ...
      const timePassed = vm.createdAt
        ? Math.floor(
            (Date.now() - new Date(vm.createdAt).getTime()) / 1000 / 60,
          )
        : 0;
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Dialog
              open={isTopUpOpen}
              onOpenChange={(o) => {
                if (txStatus !== "submitted") setIsTopUpOpen(o);
              }}
            >
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Add funds to your escrow account to continue running your
                    VM.
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  <div>
                    ...
                    <Input
                      ...
                      value={topUpAmount}
                      onChange={(e) =>
                        setTopUpAmount(parseFloat(e.target.value) || 0.1)
                      }
                      className="mt-2"
                      disabled={txStatus === "submitted"}
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      ≈ {(additionalEscrowDuration / 1440).toFixed(3)}{" "}
                      additional days of runtime
                    </p>
                  </div>
          ...
          <div className="space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              ≈ {Math.floor(timeRemaining / 1000 / 60 / 1440)} days remaining at
              current rate
            </div>
          </div>
        </CardContent>
